...
├── scripts/deploy.js
├── test/DeviceRegistry.js
├── artifacts/… (auto-generated by Hardhat)
├── env.sample
├── frontend/ (Create React App + Tailwind)
│   ├── src/components/*.jsx
│   ├── src/hooks/useDeviceRegistry.js
│   ├── src/utils/contract.js
│   └── src/abi/DeviceRegistry.json
└── README.md
```
...
2. In another terminal, deploy:
   ...
   ```
   The script prints the deployed `DeviceRegistry` address—copy it.
...
1. **Connect Wallet** – uses MetaMask through `BrowserProvider`. Displays the connected address.
2. **Register Device** – enter a device name, the UI previews `deviceId = keccak256(owner, name)`, submits to the contract, and shows a success toast.
3. **Fetch Device by ID** – paste any device ID to retrieve details (name, owner, timestamp).
4. **List My Devices** – loads every device owned by the connected account and renders cards.

### MetaMask Instructions

1. Open MetaMask → add the network you intend to use:
...
- `contracts/DeviceRegistry.sol` – core smart contract with mappings, events, and view functions.
- `test/DeviceRegistry.js` – full Chai test suite covering registration, duplicates, multi-owner support, and device listings.
- `scripts/deploy.js` – Hardhat deployment script with `ethers v6` factories.
- `frontend/src/hooks/useDeviceRegistry.js` – abstracts MetaMask connection + contract calls.
- `frontend/src/components/*` – reusable UI components (connect button, forms, lists, notifications).

## Notes

- All smart-contract data lives fully on-chain—no centralized DB or backend services.
